...
Order Analyst        Reads 20 orders of purchase history. Identifies staples
                     ...
                     with behavioral evidence for each item.

Meal Planner         Takes the user's meal requests ("lasagna Wednesday, friends
                     Saturday") and maps them to real Picnic recipes and products.
                     Cross-references with the base cart to avoid duplicates.
                     Adjusts portions for guest events.
...
User: "Sort this week's shop, lasagna Wed, friends Sat, under 80 euro"
  |
  v
[Intent Parser] -------- LLM call: extract meals, guests, budget, requests
  |
  v
...
[Preference Learning] -- Derive and persist preferences             <10ms
  ...
  |                       delivery preferences, always-include items)
  |
  v
[SSE Stream] ----------> Split-panel React UI
                          Left: grocery cart with prices + reasoning chips
                          Right: agent activity feed (color-coded, animated)
```
...
**Accumulated Memory** -- `data/preferences.json` persists learned preferences across sessions. After each run, the system derives and saves: brand preferences (from substitution acceptance/rejection), delivery day and time preferences, budget patterns, always-include items, and never-suggest items. Subsequent runs load these preferences into agent prompts, so the system improves with use.

**Periodic Autonomy** -- The system supports a proactive "auto mode" trigger. When the user provides no specific meal requests or guest events, the agent autonomously builds a cart from purchase patterns alone -- it knows what you need before you say it. The architecture supports a scheduled reorder notification after 7+ days since the last order.

**Social Context** -- The five agents coordinate through structured data handoffs. Each handoff is a typed summary emitted as an SSE event: Order Analyst passes recommended items and estimated cost to the Orchestrator, Meal Planner passes meals and ingredients to the Budget Optimizer, Schedule Agent passes the selected slot. These handoffs are visible in the activity feed, making agent-to-agent communication transparent to the user and judges.
...
| **Total (with budget opt)** | **~12-18s** | End-to-end, user input to final cart   |
| **Total (under budget)**   | **~9-12s**  | Skips budget optimizer entirely        |
