#!/bin/sh
# Copyright (c) 2026 Picnic Technologies B.V. All rights reserved.
# install.sh — Cross-platform installer for picnic-api-skill
#
...
install_files() {
    ...
    case "$INSTALL_DIR" in
        /|"$HOME"|"$HOME/"|/tmp|/tmp/|/var|/var/|/etc|/etc/|/usr|/usr/)
            error "Refusing to install to dangerous path: ${INSTALL_DIR}"
            exit 1 ;;
    esac
    depth=$(echo "$INSTALL_DIR" | tr '/' '\n' | grep -c '.')
    if [ "$depth" -lt 3 ]; then
        error "Install path suspiciously shallow (safety check failed): ${INSTALL_DIR}"
        exit 1
    fi
...
install_all() {
    ...
    for plat in $ALL_PLATFORMS; do
        ...
        installed_names="${installed_names} ${plat}"
        print_activation_instructions
    ...
    if $DRY_RUN; then
        ...
        success "Skill '${SKILL_NAME}' installed to ${installed_count} platform(s):${installed_names}"
    fi
}
